...
func GenerateGoContent(errors map[string]ErrorDefinition) (string, error) {
	...
	builder.WriteString("\treturn fmt.Sprintf(\"[Domain: %s] [Code: %s] Msg: %s | Cause: %s | Severity: %s | Retryable: %t\",\n")
	builder.WriteString("\t\te.Domain, e.Code, e.Msg, e.Cause, e.Severity, e.IsRetryable)\n")
	builder.WriteString("}\n\n")
